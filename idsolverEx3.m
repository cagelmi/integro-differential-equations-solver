function nominales = idsolverEx3(lim,n,IC,c,d,k,alpha,beta,Tol,Flag)
% IDSOLVEREX3 solves Example 3, which is an integro-differential equation
% with a quadratic kernel.
%
...
dy(n) = c(x,y(1)) + d(x)*quadl(@(s) k(x,s).*ys(s).^2,alpha(x),beta(x),TolQuad);